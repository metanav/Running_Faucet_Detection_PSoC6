...
void audio_task(void *arg)
{
    ...
    for(;;)
    {
        ...
        int16_t *audio_buf = audio_rec_get_new_buffer();
        if(audio_buf != NULL){
          int model_result = sml_recognition_run(audio_buf, AUDIO_REC_RECORD_BUFFER_SIZE, 1);
          class_t cls;
          cls.index = model_result;
  	      if (xQueueSendToBack(xQueue, &cls, 0) != pdPASS) {
              printf("xQueue: no queue space\r\n");
          }
        }
    }
