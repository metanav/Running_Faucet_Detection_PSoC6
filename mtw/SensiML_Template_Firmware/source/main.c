...
#define OLED_TASK_STACK_SIZE        (1024*2)
...
int main(void)
{
    ...
    if(CY_RSLT_SUCCESS != cy_retarget_io_init(CYBSP_DEBUG_UART_TX, CYBSP_DEBUG_UART_RX, CY_RETARGET_IO_BAUDRATE))
    {
        ...
        xQueue = xQueueCreate(50, sizeof(class_t));

        /* Create the MQTT Client task. */
        xTaskCreate(mqtt_client_task, "MQTT Client task", MQTT_CLIENT_TASK_STACK_SIZE,
                    NULL, MQTT_CLIENT_TASK_PRIORITY, NULL);
        /* Create the OLED task */
        xTaskCreate( oled_task, "OLED task", OLED_TASK_STACK_SIZE,  NULL,
                         OLED_TASK_PRIORITY,  NULL);
        /* Create the Audio task */
        xTaskCreate(audio_task, "Audio task", TASK_STACK_SIZE, NULL, TASK_PRIORITY, NULL);
    #endif
